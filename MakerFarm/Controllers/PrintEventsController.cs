    ...
    public class PrintEventsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "PrintEventId,EventType,MaterialUsed,PrinterId,UserName,PrintId,PrintErrorTypeId,Comment")] PrintEvent printevent)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if(!printevent.EventType.Equals(PrintEventType.PRINT_START) && !(printevent.EventType.Equals(PrintEventType.PRINT_CANCELED) && (LastEvent != null && LastEvent.EventType == PrintEventType.PRINT_START)) && !db.Printers.Find(printevent.PrinterId).PrinterName.Equals("Null Printer"))
                {
